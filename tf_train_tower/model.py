...
class Model(tf.keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        lhuc_input = deep
        deep1 = self.lhuc_net(deep, [2048,1024], 'deep_lhuc1', lhuc_input, [128], self.dropout_dim, self.training, model_conf.use_bn)
        deep2 = self.lhuc_net(deep, [2048,1024], 'deep_lhuc2', lhuc_input, [128], self.dropout_dim, self.training, model_conf.use_bn)
        deep3 = self.lhuc_net(deep, [2048,1024], 'deep_lhuc3', lhuc_input, [128], self.dropout_dim, self.training, model_conf.use_bn)
        deep4 = self.lhuc_net(deep, [2048,1024], 'deep_lhuc4', lhuc_input, [128], self.dropout_dim, self.training, model_conf.use_bn)
        ...
        buy_tower_output = self.task_tower(buy_output, "buy_tower", self.training, [256,64], [tf.nn.relu, tf.nn.relu], self.dropout_dim)
        cat_tower_output = self.task_tower(cat_output, "cat_tower", self.training, [256,64], [tf.nn.relu, tf.nn.relu], self.dropout_dim)
        click_tower_output = self.task_tower(clk_output, "click_tower", self.training, [256,64], [tf.nn.relu, tf.nn.relu], self.dropout_dim)
        ext_tower_output = self.task_tower(ext_output, "ext_tower", self.training, [256,64], [tf.nn.relu, tf.nn.relu], self.dropout_dim)
