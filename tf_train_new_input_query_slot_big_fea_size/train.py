...
class Learner:
    ...
    def train(self, train_data, start_day, end_day, model_path=None, data_path=None):
        ...
        # [加速] 把forward+backward抽成独立函数，加tf.function减少shape推断开销
        @tf.function(experimental_relax_shapes=True)
        def train_step(sids, fids, cvr_label, cat_label, clk_label, ext_label):
            with tf.GradientTape() as tape:
                pred_buy, pred_cat, pred_click, pred_ext, gate1, gate2, gate3, gate4 = model([sids, fids])
                loss_buy   = model.loss(tf.expand_dims(cvr_label, 1), pred_buy)
                loss_cat   = model.loss(tf.expand_dims(cat_label, 1), pred_cat)
                loss_click = model.loss(tf.expand_dims(clk_label, 1), pred_click)
                loss_ext   = model.loss(tf.expand_dims(ext_label, 1), pred_ext)
                final_loss = (loss_buy   * buy_weight +
                                loss_cat   * cat_weight +
                                loss_click * click_weight +
                                loss_ext   * ext_weight)
                gradients = tape.gradient(final_loss, model.trainable_weights)
            model.optimizer.apply_gradients(zip(gradients, model.trainable_weights))
            return loss_buy, loss_cat, loss_click, loss_ext, final_loss, gate1, gate2, gate3, gate4
        print('training...')
        for epo in range(epoch_num):
            ...
            for step, feat in enumerate(train_data):
                cnt       += int(tf.shape(feat['cvr_label'])[0])
                pos       += int(tf.reduce_sum(feat['cvr_label']))
                pos_cat   += int(tf.reduce_sum(feat['cat_label']))
                pos_click += int(tf.reduce_sum(feat['clk_label']))
                pos_ext   += int(tf.reduce_sum(feat['ext_label']))

                loss_buy, loss_cat, loss_click, loss_ext, final_loss, gate1, gate2, gate3, gate4 = train_step(
                    feat['sids'], feat['fids'],
