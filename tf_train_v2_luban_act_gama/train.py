...
class Learner:
    ...
    def train(self, train_data, start_day, end_day, model_path=None, data_path=None):
        ...
        def fusion_Loss(fusion_score, feat, w_buy, w_cat, w_click, w_ext):
            loss_buy = model.loss(tf.expand_dims(feat['cvr_label'], 1), fusion_score)
            loss_cat = model.loss(tf.expand_dims(feat['cat_label'], 1), fusion_score)
            loss_click = model.loss(tf.expand_dims(feat['clk_label'], 1), fusion_score)
            loss_ext = model.loss(tf.expand_dims(feat['ext_label'], 1), fusion_score)
            return w_buy*loss_buy + w_cat*loss_cat + w_click*loss_click + w_ext*loss_ext
        ...
        for epo in range(epoch_num):
            ...
            for step, feat in enumerate(train_data):
                with tf.GradientTape() as tape:
                    ...
                    pred_buy, pred_cat, pred_click, pred_ext, gate1, gate2, gate3, gate4, fusion_score, fusion_score1, fusion_score2, fusion_score3, fusion_score4, fusion_score5, fusion_score6, fusion_score7, fusion_score8, fusion_score9  = model([feat['sids'], feat['fids']])

                    #pred_buy, pred_cat, pred_click, pred_ext, gate1, gate2, gate3 , gate4, fscore = model([feat['sids'], feat['fids']])
                    ...
                    floss  =  fusion_Loss(fusion_score,  feat, 1.0, 0.0, 0.01, 0.0)
                    f1loss =  fusion_Loss(fusion_score1, feat, 1.0, 0.0, 0.02, 0.0)
                    f2loss =  fusion_Loss(fusion_score2, feat, 1.0, 0.0, 0.03, 0.0)
                    f3loss =  fusion_Loss(fusion_score3, feat, 1.0, 0.0, 0.04, 0.0)
                    f4loss =  fusion_Loss(fusion_score4, feat, 1.0, 0.0, 0.05, 0.0)

                    f5loss =  fusion_Loss(fusion_score5, feat, 0.01, 0.0, 1.0, 0.0)
                    f6loss =  fusion_Loss(fusion_score6, feat, 0.02, 0.0, 1.0, 0.0)
                    f7loss =  fusion_Loss(fusion_score7, feat, 0.03, 0.0, 1.0, 0.0)
                    f8loss =  fusion_Loss(fusion_score8, feat, 0.04, 0.0, 1.0, 0.0)
                    f9loss =  fusion_Loss(fusion_score9, feat, 0.15, 0.0, 1.0, 0.0)

                    final_loss = loss_buy * buy_weight + loss_cat * cat_weight + loss_click * click_weight + loss_ext * ext_weight + floss + f1loss + f2loss + f3loss + f4loss + f5loss+ f6loss + f7loss + f8loss + f9loss
                    ...
                    if step % 100 == 0:
                        ...
                        # dfm.save_weights(os.path.join(model_dir, 'dfm_%08d.h5'%step))
    ...
    def dump_serving_model(self, end_day, epo):
        ...
        serve_model.save(
            'serving_model_%s/%s30' % (epo, end_day),
            save_format='tf'
        )
