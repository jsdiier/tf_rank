...
class Learner:
    ...
    def test(self, test_data, model_path=''):
        ...
        for step, feat in enumerate(test_data):
            ...
            pred_buy, pred_cat, pred_click, pred_ext, gate1, gate2, gate3, gate4, fusion_score, fusion_score1, fusion_score2, fusion_score3, fusion_score4, fusion_score5, fusion_score6, fusion_score7, fusion_score8, fusion_score9  = self.model([feat['sids'], feat['fids']])
            ...
            fusion_score9 = tf.squeeze(fusion_score9, 1).numpy().tolist()
            ...
            clk_res_buy.extend(zip(buy_label, fusion_score9, recID, uid, score, rank))
            res_cat.extend(zip(cat_label, pred_cat, recID, uid, score, rank))
            res_click.extend(zip(click_label, fusion_score9, recID, uid, score, rank))
        ...
        auc_score, group_auc, u_avg_auc, o_auc_score, o_group_auc, o_u_avg_auc = ut.multi_auc(clk_res_buy)
        print(model_path, "test_clk_buy auc:%f gauc:%f uauc:%f size:%d loss:%f, pos: %d" % (
            auc_score, group_auc, u_avg_auc, len(res_buy), loss_buy_sum / len(res_buy), pos_buy))
