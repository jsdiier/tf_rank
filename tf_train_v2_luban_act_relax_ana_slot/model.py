...
class FusionPolicy(tf.keras.Model):

    def __init__(self, name="policy"):
        super().__init__(name=name)

        self.fusion1 = tf.keras.layers.Dense(128, activation='relu')
        self.fusion2 = tf.keras.layers.Dense(64, activation='relu')
        self.out = tf.keras.layers.Dense(1, activation='sigmoid')

    def call(self, buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output):
        x = tf.concat([
            tf.stop_gradient(buy_tower_output),
            tf.stop_gradient(cat_tower_output),
            tf.stop_gradient(click_tower_output),
            tf.stop_gradient(ext_tower_output)
        ], axis=-1)
...
class Model(tf.keras.Model):
    def __init__(self, training=False, pred=False, fid_kv=None, fid_ads_kv=None):
        ...
        self.fusion_policy = FusionPolicy(name="base")
        self.fusion_policy_ref1 = FusionPolicy(name="ref1")
        self.fusion_policy_ref2 = FusionPolicy(name="ref2")
        self.fusion_policy_ref3 = FusionPolicy(name="ref3")
        self.fusion_policy_ref4 = FusionPolicy(name="ref4")
        self.fusion_policy_ref5 = FusionPolicy(name="ref5")
        self.fusion_policy_ref6 = FusionPolicy(name="ref6")
        self.fusion_policy_ref7 = FusionPolicy(name="ref7")
        self.fusion_policy_ref8 = FusionPolicy(name="ref8")
        self.fusion_policy_ref9 = FusionPolicy(name="ref9")
        #embedding table
        ...
        # precomputed static flat dims, used to reshape as [-1, KNOWN_DIM] instead of [tf.shape(x)[0], -1]
        # (mixing a dynamic batch tensor with -1 in the same reshape call breaks static shape inference
        # for the other dimension too, which is what caused the "Dense layer input last dim is None" crash)
        self._query_flat_dim = len(model_conf.query_slots) * (model_conf.lr_emb_size + model_conf.fm_emb_size - 1)
        self._all_emb_flat_dim = len(model_conf.embedding_slot_ids) * (model_conf.lr_emb_size + model_conf.fm_emb_size)
        self._ads_emb_flat_dim = len(model_conf.ads_fea_slots) * model_conf.din_emb_size
    ...
    def lhuc_net(self, nn_input, nn_dims, name, lhuc_input, lhuc_basic_dims, dropout_dim, is_train, use_bn=False):
        ...
        if layer_key not in self.lhuc_layers_cache:
            ...
            if use_bn:
                bn_layer = tf.keras.layers.BatchNormalization(name=name+'_bn', momentum=model_conf.bn_momentum)
    ...
    def attention_din_ads(self, query, key, mask, ads_emb, name, att_hidden_units):
        ads_key = self.ads_seq_cross_layer(name=name+'_ads_layer',
                    ...
                    ads_emb=ads_emb,
                    ads_hidden_dim=64,
                    ads_output_dim=key.get_shape().as_list()[-1])
    ...
    def call(self, inputs, training=None):
        ...
        ads_emb = tf.reshape(ads_emb, [-1, self._ads_emb_flat_dim])
        ...
        fusion_score = self.fusion_policy(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)
        fusion_score1 = self.fusion_policy_ref1(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)
        fusion_score2 = self.fusion_policy_ref2(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)
        fusion_score3 = self.fusion_policy_ref3(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)
        fusion_score4 = self.fusion_policy_ref4(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)
        fusion_score5 = self.fusion_policy_ref5(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)
        fusion_score6 = self.fusion_policy_ref6(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)
        fusion_score7 = self.fusion_policy_ref7(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)
        fusion_score8 = self.fusion_policy_ref8(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)
        fusion_score9 = self.fusion_policy_ref9(buy_tower_output, cat_tower_output, click_tower_output, ext_tower_output)

        if self.is_save_model or self.pred:
            final_pred = buy_pred
            return buy_pred, cat_pred, fusion_score9*1.133405, ext_pred
        return buy_pred, cat_pred, click_pred, ext_pred,gate1, gate2, gate3, gate4, fusion_score, fusion_score1, fusion_score2, fusion_score3, fusion_score4, fusion_score5, fusion_score6, fusion_score7, fusion_score8, fusion_score9
